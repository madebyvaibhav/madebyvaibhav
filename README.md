<div align="center">

# <img src="https://raw.githubusercontent.com/ABSphreak/ABSphreak/master/gifs/Hi.gif" width="32px" alt="Wave"> Hi, I'm **Vaibhav Thakur**

<p align="center">
  <b>Full-Stack Developer · AI / ML Enthusiast · Unity Game Creator · Open Source Contributor</b>
</p>

<!-- Typing SVG Animation -->
<a href="https://github.com/madebyvaibhav">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=20&duration=2500&pause=800&color=8B5CF6&center=true&vCenter=true&width=700&lines=Building+the+future+with+code+%26+creativity;Crafting+Full-Stack+Web+%26+AI+Applications;Designing+Immersive+3D+Games+in+Unity;Architecting+Scalable+System+Designs" alt="Typing Intro" />
</a>

<br/>
<br/>

<p align="center">
  <a href="https://github.com/madebyvaibhav"><img src="https://img.shields.io/github/followers/madebyvaibhav?label=Followers&style=for-the-badge&color=8B5CF6&labelColor=0D1117&logo=github" alt="Followers" /></a>
  <a href="https://komarev.com/ghpvc/?username=madebyvaibhav&style=for-the-badge&color=06B6D4&label=PROFILE+VIEWS"><img src="https://komarev.com/ghpvc/?username=madebyvaibhav&style=for-the-badge&color=06B6D4&label=PROFILE+VIEWS" alt="Profile Views" /></a>
  <a href="https://github.com/madebyvaibhav?tab=repositories"><img src="https://img.shields.io/github/stars/madebyvaibhav?label=Total%20Stars&style=for-the-badge&color=EC4899&labelColor=0D1117&logo=github" alt="Stars" /></a>
  <img src="https://img.shields.io/badge/Status-Building_%26_Learning-10B981?style=for-the-badge&logo=statuspage&logoColor=white" alt="Status" />
</p>

</div>

---

## ⚡ `> whoami`

```typescript
const vaibhav = {
  name: "Vaibhav Thakur",
  role: "Full-Stack Developer & AI Innovator",
  location: "Gujarat, India 🇮🇳",
  education: "Computer Engineering Student @ SVMIT",
  currentFocus: ["Building Ira AI Voice Assistant", "College ERP Platform", "System Design & DSA"],
  passions: ["Artificial Intelligence", "Unity Game Engines", "Open Source Architecture"],
  funFact: "I debug with 90% confidence and 10% Stack Overflow & AI ⚡"
};
```

---

## 💡 What I Do

<table align="center" width="100%">
  <tr>
    <td width="50%" valign="top">
      <h3>🌐 Web & Full-Stack Development</h3>
      <ul>
        <li>Building high-performance React & Next.js applications</li>
        <li>Architecting fast REST APIs with Python & FastAPI</li>
        <li>Designing modern glassmorphism & responsive interfaces</li>
      </ul>
    </td>
    <td width="50%" valign="top">
      <h3>🤖 AI, Voice & Security Tools</h3>
      <ul>
        <li>Developing intelligent voice assistants with speech recognition</li>
        <li>Building OSINT & phishing detection security scanners</li>
        <li>Integrating Machine Learning models into web workflows</li>
      </ul>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3>🎮 Unity Game Development</h3>
      <ul>
        <li>Crafting 3D Business Simulators & Tycoon games</li>
        <li>C# scripting, physics, and gameplay mechanics</li>
        <li>Designing interactive levels and asset pipelines</li>
      </ul>
    </td>
    <td width="50%" valign="top">
      <h3>🧠 Data Structures & System Design</h3>
      <ul>
        <li>Daily problem-solving on LeetCode & competitive platforms</li>
        <li>Studying scalable microservices & distributed architectures</li>
        <li>Optimizing code complexity and algorithmic efficiency</li>
      </ul>
    </td>
  </tr>
</table>

---

## 🛠️ Tech Stack & Arsenal

<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,ts,react,nextjs,tailwind,python,fastapi,nodejs,mongodb,postman,unity,cs,cpp,git,github,vscode,vercel,docker,figma&theme=dark" alt="Tech Stack Icons" />
</p>

<details open>
<summary><b>🔍 Detailed Categorized Breakdown</b></summary>
<br/>

| Category | Technologies |
|---|---|
| **Frontend** | `HTML5` `CSS3` `JavaScript (ES6+)` `TypeScript` `React.js` `Next.js` `Tailwind CSS` |
| **Backend & APIs** | `Python` `FastAPI` `Node.js` `Express.js` `C++` `REST APIs` |
| **AI / ML & Security** | `Speech Recognition` `OSINT Tools` `OpenCV` `PyTorch / AI Models` |
| **Game Development** | `Unity 3D` `C#` `Game Physics` `UI Canvas` |
| **Databases & DevOps** | `MongoDB` `PostgreSQL` `Firebase` `Git` `GitHub Actions` `Docker` `Vercel` |

</details>

---

## 🚀 Featured Projects

| Project | Description | Stack / Badges | Links |
| :--- | :--- | :--- | :---: |
| 🤖 **Ira AI Assistant** | Voice assistant with speech recognition, task automation & smart scheduling | `TypeScript` `AI` `Speech API` | [<img src="https://img.shields.io/badge/View_Repo-8B5CF6?style=flat-square&logo=github&logoColor=white" height="22"/>](https://github.com/madebyvaibhav/ira-ai-assistant) |
| 🎓 **College Management System** | Full-stack college ERP with timetable, attendance & role dashboards | `Python` `FastAPI` `Full-Stack` | [<img src="https://img.shields.io/badge/View_Repo-38BDF8?style=flat-square&logo=github&logoColor=white" height="22"/>](https://github.com/madebyvaibhav/college-management-system) |
| 🛡️ **Cyber Kavach** | Security scanner flagging phishing links & fake images for companies | `Python` `Cybersecurity` `OSINT` | [<img src="https://img.shields.io/badge/View_Repo-EC4899?style=flat-square&logo=github&logoColor=white" height="22"/>](https://github.com/madebyvaibhav/cyber-kavach) |
| 🇮🇳 **Indian Startup Tycoon** | 30 Unity Business Tycoon games — Build your Indian Startup Empire | `Unity` `C#` `GameDev` | [<img src="https://img.shields.io/badge/View_Repo-10B981?style=flat-square&logo=github&logoColor=white" height="22"/>](https://github.com/madebyvaibhav/indian-startup-tycoon) |
| 🌐 **Portfolio Website** | Personal portfolio website built with Next.js and Tailwind CSS | `Next.js` `React` `Tailwind` | [<img src="https://img.shields.io/badge/View_Repo-F59E0B?style=flat-square&logo=github&logoColor=white" height="22"/>](https://github.com/madebyvaibhav/portfolio-website) |

---

## 📊 Analytics & Activity

<p align="center">
  <img height="180" src="https://github-readme-stats.vercel.app/api?username=madebyvaibhav&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=8B5CF6&icon_color=06B6D4&text_color=C9D1D9" alt="GitHub Stats" />
  <img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=madebyvaibhav&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=8B5CF6&text_color=C9D1D9" alt="Top Languages" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=madebyvaibhav&theme=tokyonight&hide_border=true&background=0D1117&ring=8B5CF6&fire=06B6D4&currStreakLabel=C9D1D9" alt="GitHub Streak" />
</p>

---

## 🏆 GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=madebyvaibhav&theme=tokyonight&no-frame=true&row=1&column=7" alt="GitHub Trophies" />
</p>

---

## 🎯 Roadmap & Goals

- [x] Architect & Deploy Profile Readme & Portfolio Repositories
- [ ] Build and launch **Ira AI Voice Assistant v2**
- [ ] Master Advanced System Design & Microservice Architectures
- [ ] Solve 300+ LeetCode DSA Problems
- [ ] Target Placement @ Google (2028 Batch) 🎯

---

## 🤝 Let's Connect & Collaborate

<p align="center">
  <a href="https://linkedin.com/in/madebyvaibhav"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/></a>
  <a href="https://vaibhavthakur.vercel.app"><img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio"/></a>
  <a href="mailto:vaibhavthakur007@gmail.com"><img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail"/></a>
  <a href="https://twitter.com/madebyvaibhav"><img src="https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white" alt="Twitter"/></a>
  <a href="https://instagram.com/vaibhav_16_1"><img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="Instagram"/></a>
</p>

---

<div align="center">

<img src="https://raw.githubusercontent.com/madebyvaibhav/madebyvaibhav/output/github-contribution-grid-snake.svg" alt="Snake animation" />

<br/>

<i>"Building the future with code, passion, and creativity."</i>  
**Made with 💜 by Vaibhav Thakur**

</div>
